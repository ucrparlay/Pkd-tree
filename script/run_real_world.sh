...
k
		...
		#perf record -s -o ${perf_data_name} -e ${events} -D -1 --control fd:${ctl_fd},${ctl_fd_ack} ${exe} -p ${basepath}/${ds}.in -k 10 -t 65536 -d ${dim} -q 4 -r 1 -pcf ${ctl_fd} -pcaf ${ctl_fd_ack} > ${logdir}/${logname} 2>&1
		...
		#perf record -o ${perf_data_name} -e ${events} ${exe} -p ${basepath}/${ds}.in -k 10 -t 65536 -d ${dim} -q 4 -r 1 > ${logdir}/${logname} 2>&1

		#${exe} -p ${basepath}/${ds}.in -k 10 -t 65536 -d ${dim} -q 4 -r 1 -perf "-o ${perf_data_name} -e ${events}" > ${logdir}/${logname} 2>&1

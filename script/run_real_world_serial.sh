...
for queryType in ${QueryTypes[@]}; do
    ...
    if [[ ${queryType} == 7 ]]; then
        resFile="perf_real_gen_serial_bb.out"
    elif [[ ${queryType} == 15 ]]; then
        resFile="perf_real_total_serial_bb.out"
    fi
    ...
    for solver in ${Solvers[@]}; do
        ...
        for filename in "${!file2Dims[@]}"; do
            ...
            perf_data_name="${perf_path}/${solver}_${filename}_perf_bb.data"
            perf_report_name="${perf_path}/${solver}_${filename}_perf_bb.report"

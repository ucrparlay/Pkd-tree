...
//@ cut and then rotate
template<typename point>
void ParallelKDtree<point>::divide_rotate( slice In, splitter_s& pivots, int dim, int idx,
                                           int deep, int& bucket, const int& DIM ) {
...
template<typename point>
void ParallelKDtree<point>::start_build( slice A, const int& DIM ) {
  points B = points::uninitialized( A.size() );
  node* root = build( A, B.cut( 0, A.size() ), 0, DIM );
  set_root( root );
  return;
}

template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::build( slice In, slice Out,
                                                                    int dim,
                                                                    const int& DIM ) {
  size_t n = In.size();
...
//? parallel query
template<typename point>
void ParallelKDtree<point>::k_nearest( node* T, const point& q, const int& DIM,

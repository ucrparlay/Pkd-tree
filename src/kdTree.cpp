...
template<typename T>
KDnode<T>* KDtree<T>::init(const int& _DIM, const int& _LEAVE_WRAP, Point<T>* a, int len) {
  this->DIM = _DIM;
...
template<typename T>
KDnode<T>* KDtree<T>::make_tree(Point<T>* a, int len, int i) {
...
template<typename T>
void KDtree<T>::k_nearest(KDnode<T>* root, Point<T>* nd, int i, kBoundedQueue<T>& q) {
  T d, dx, dx2;
  // root->print();
  ...
  if (Gt(dx2, q.top()) && q.full()) return;
  k_nearest(dx > 0 ? root->right : root->left, nd, i, q);
}

template<typename T>
void KDtree<T>::k_nearest_array(KDnode<T>* root, Point<T>* nd, int i, kArrayQueue<T>& kq) {
  T d, dx, dx2;
  // root->print();
  ...
  if (Gt(dx2, kq.queryKthElement()) && kq.getLoad() >= this->K) return;
  k_nearest_array(dx > 0 ? root->right : root->left, nd, i, kq);
}

template<typename T>
void KDtree<T>::destory(KDnode<T>* root) {
  if (root->isLeaf) {
    delete[] root->p;
    return;
  }

...
void testParallelkArray(int Dim, int LEAVE_WRAP, Point<Typename>* wp, int N, int K) {
  ...
  timer.reset();
  timer.start();
  parlay::parallel_for(0, N, [&](size_t i) { KD.k_nearest_array(KDroot, &wp[i], 0, kq[i]); });
...
void testParallel(int Dim, int LEAVE_WRAP, Point<Typename>* wp, int N, int K) {
  parlay::internal::timer timer;
  KDtree<Typename> KD;
  timer.start();
  KDnode<Typename>* KDroot = KD.init(Dim, LEAVE_WRAP, wp, N);
  ...
  timer.reset();
  timer.start();
  parlay::parallel_for(0, N, [&](size_t i) { KD.k_nearest(KDroot, &wp[i], 0, bq[i]); });
...
int main(int argc, char* argv[]) {
  ...
  if (argc >= 5) LEAVE_WRAP = std::stoi(argv[4]);

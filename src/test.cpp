...
int main( int argc, char* argv[] ) {
    ...
    if ( tag == -1 ) {
        ...
        decltype( wp )().swap( wp );
        testParallelKDtree<point2D>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile,
                                     tag );
    } else if ( Dim == 3 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> point3D { return point3D( wp[i].pnt.begin() ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<point3D>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile,
                                     tag );
    } else if ( Dim == 5 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> point5D { return point5D( wp[i].pnt.begin() ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<point5D>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile,
                                     tag );
    } else if ( Dim == 7 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> point7D { return point7D( wp[i].pnt.begin() ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<point7D>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile,
                                     tag );
    }

...
class ParallelKDtree {
 ...
 public:
  ...
  void divide_rotate( slice In, splitter_s& pivots, int dim, int idx, int deep,
                      int& bucket, const int& DIM );

  template<typename slice>
  std::array<uint_fast32_t, PIVOT_NUM> partition(
      slice A, slice B, const size_t& n, const std::array<coord, PIVOT_NUM>& pivots,
      const int& dim );

  void pick_pivots( slice In, const size_t& n, splitter_s& pivots, const int& dim,
                    const int& DIM );
  ...
  void partition( slice A, slice B, const size_t& n, const splitter_s& pivots,
                  parlay::sequence<uint_fast32_t>& sums, const int& dim, const int& DIM );

  node* build_inner_tree( uint_fast16_t idx, splitter_s& pivots,
                          parlay::sequence<node*>& treeNodes );

  inline coord ppDistanceSquared( const point& p, const point& q, const int& DIM );

  //@ Parallel KD tree cores
  node* build( slice In, slice Out, int dim, const int& DIM );

  void k_nearest( node* T, const point& q, const int& DIM, kBoundedQueue<coord>& bq,
                  size_t& visNodeNum );

...
bool ParallelKDtree<point>::checkBox(node* T, const box& bx) {
    assert(T != nullptr);
    assert(legal_box(bx));
...
template<typename point>
size_t ParallelKDtree<point>::checkSize(node* T) {
    if (T->is_leaf) {
        return T->size;
    }
...
template<typename point>
void ParallelKDtree<point>::checkTreeSameSequential(node* T, int dim, const int& DIM) {
    if (T->is_leaf) {
...
template<typename point>
void ParallelKDtree<point>::validate(const dim_type DIM) {
...
template<typename point>
size_t ParallelKDtree<point>::getTreeHeight() {
    size_t deep = 0;
    return getMaxTreeDepth(this->root, deep);
}

template<typename point>
size_t ParallelKDtree<point>::getMaxTreeDepth(node* T, size_t deep) {
    if (T->is_leaf) {
        return deep;
    }
    interior* TI = static_cast<interior*>(T);
...
template<typename point>
double ParallelKDtree<point>::getAveTreeHeight() {
...
template<typename point>
size_t ParallelKDtree<point>::countTreeNodesNum(node* T) {
    if (T->is_leaf) {
        return 1;
    }
    ...
    interior* TI = static_cast<interior*>(T);
    size_t l, r;
    parlay::par_do([&]() { l = countTreeNodesNum(TI->left); }, [&]() { r = countTreeNodesNum(TI->right); });
    return l + r + 1;
}

template<typename point>
void ParallelKDtree<point>::countTreeHeights(node* T, size_t deep, size_t& idx, parlay::sequence<size_t>& heights) {
    if (T->is_leaf) {
        heights[idx++] = deep;
        return;
    }

...
template<typename point>
typename ParallelKDtree<point>::leaf* ParallelKDtree<point>::alloc_leaf_node(
    slice In) {
...
template<typename point>
typename ParallelKDtree<point>::leaf* ParallelKDtree<point>::alloc_dummy_leaf(
    slice In) {
    leaf* o = parlay::type_allocator<leaf>::alloc();
    new (o) leaf(In, true);
...
template<typename point>
inline bool ParallelKDtree<point>::inbalance_node(const size_t l,
                                                  const size_t n) {
    if (n == 0)
        return false;  // BUG: this will disable parallel delete a tree in batch
                       // deletion when the tree was emptied

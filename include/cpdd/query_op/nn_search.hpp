...
inline typename ParallelKDtree<point>::coord ParallelKDtree<point>::p2p_distance(const point& p, const point& q,
                                                                                 const dim_type DIM) {
    coord r = 0;
...
template<typename point>
inline typename ParallelKDtree<point>::coord ParallelKDtree<point>::p2b_min_distance(
    const point& p, const typename ParallelKDtree<point>::box& a, const dim_type DIM) {
    coord r = 0;
    for (dim_type i = 0; i < DIM; i++) {
...
template<typename point>
inline typename ParallelKDtree<point>::coord ParallelKDtree<point>::p2b_max_distance(
    const point& p, const typename ParallelKDtree<point>::box& a, const dim_type DIM) {
    coord r = 0;
    for (dim_type i = 0; i < DIM; i++) {
...
template<typename point>
inline typename ParallelKDtree<point>::coord ParallelKDtree<point>::interruptible_distance(const point& p,
                                                                                           const point& q, coord up,
                                                                                           dim_type DIM) {
    coord r = 0;
    dim_type i = 0;
    if (DIM >= 6) {
...
void ParallelKDtree<point>::k_nearest(node* T, const point& q, const dim_type DIM, kBoundedQueue<point, StoreType>& bq,
    ...
    auto go_left = [&]() -> bool { return Num::Gt(TI->split.first - q.pnt[TI->split.second], 0); };

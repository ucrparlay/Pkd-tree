...
size_t ParallelKDtree<point>::range_count(const typename ParallelKDtree<point>::box& bx, size_t& visLeafNum,
                                          size_t& visInterNum) {
...
template<typename point>
size_t ParallelKDtree<point>::range_count(const circle& cl) {
    return range_count_radius(this->root, cl, this->bbox);
}

// TODO as range_count_rectangle
template<typename point>
size_t ParallelKDtree<point>::range_count_radius(node* T, const circle& cl, const box& nodeBox) {
    ...
    size_t l, r;
    parlay::par_do_if(
        TI->size >= SERIAL_BUILD_CUTOFF, [&] { l = range_count_radius(TI->left, cl, lbox); },
...
template<typename point>
ParallelKDtree<point>::simple_node* ParallelKDtree<point>::range_count_save_path(node* T, const box& queryBox,
                                                                                 const box& nodeBox) {
    ...
    simple_node *L, *R;
    parlay::par_do_if(
        TI->size >= SERIAL_BUILD_CUTOFF, [&] { L = range_count_save_path(TI->left, queryBox, lbox); },

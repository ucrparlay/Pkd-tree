...
//  If @granularity is set to true, then the flatten will switch to serial by
//  SERIAL_BUILD_CUTOFF; otherwise, always flatten in parallel
...
void ParallelKDtree<point>::flatten(typename ParallelKDtree<point>::node* T,
    ...
    parlay::par_do_if(
        ...
        (granularity && TI->size > SERIAL_BUILD_CUTOFF) ||
...
// NOTE: flatten a tree T into the range Out, afterwards delete the tree when
// recursing back
template<typename point>
...
ParallelKDtree<point>::rebuild_single_tree(node* T, const dim_type d,
                                           const dim_type DIM, size_t old_size,
    ...
    auto tmpNodeBox = node_box(std::move(o), std::move(bx));
...
ParallelKDtree<point>::rebuild_tree_recursive(node* T, dim_type d,
    ...
    parlay::par_do_if(
        ...
        // parallel
        (granularity && T->size > SERIAL_BUILD_CUTOFF) ||
            (!granularity && TI->aug_flag),
        [&] {
            std::tie(L, Lbox) =
...
void ParallelKDtree<point>::delete_tree_recursive(node* T, bool granularity) {
    ...
    if (T->is_leaf) {
        ...
        // NOTE: enable granularity control by default, if it is disabled, will
        // check granularity by the value in the tree
        parlay::par_do_if(
            (granularity && T->size > SERIAL_BUILD_CUTOFF) ||
                (!granularity && TI->aug_flag),

...
template<typename point>
void ParallelKDtree<point>::divide_rotate(slice In, splitter_s& pivots,
                                          dim_type dim, bucket_type idx,
                                          bucket_type deep, bucket_type& bucket,
                                          const dim_type DIM, box_s& boxs,
                                          const box& bx) {
  ...
  d = (d + 1) % DIM;
  divide_rotate(In.cut(0, n / 2), pivots, d, 2 * idx, deep + 1, bucket, DIM,
                boxs, lbox);
  divide_rotate(In.cut(n / 2, n), pivots, d, 2 * idx + 1, deep + 1, bucket, DIM,
                boxs, rbox);
  return;
}
...
template<typename point>
void ParallelKDtree<point>::pick_pivots(slice In, const size_t& n,
                                        splitter_s& pivots, const dim_type dim,
                                        const dim_type DIM, box_s& boxs,
                                        const box& bx) {
...
template<typename point>
inline uint_fast8_t ParallelKDtree<point>::find_bucket(
    const point& p, const splitter_s& pivots) {
  uint_fast8_t k = 1;
  while (k <= PIVOT_NUM) {
...
template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::build_inner_tree(
    bucket_type idx, splitter_s& pivots, parlay::sequence<node*>& treeNodes) {
  if (idx > PIVOT_NUM) {
...
ParallelKDtree<point>::serial_build_recursive(slice In, slice Out, dim_type dim,
  ...
  if (splitIter <= In.begin() + n / 2) {
    ...
    if (_split_rule ==
        MAX_STRETCH_DIM) {  //* next recursion redirects to new dim
      return serial_build_recursive(In, Out, d, DIM, get_box(In));
    } else if (_split_rule ==
               ROTATE_DIM) {  //* switch dim, break rotation order
...
template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::build_recursive(
    slice In, slice Out, dim_type dim, const dim_type DIM, const box& bx) {
  ...
  // NOTE: if random sampling failed to split points, re-partitions using serail
  //  approach

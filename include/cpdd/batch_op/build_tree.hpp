...
void ParallelKDtree<point>::divide_rotate(slice In, splitter_s& pivots,
    ...
    divide_rotate(In.cut(0, n / 2), pivots, d, 2 * idx, deep + 1, bucket, DIM,
                  boxs, lbox);
    divide_rotate(In.cut(n / 2, n), pivots, d, 2 * idx + 1, deep + 1, bucket,
                  DIM, boxs, rbox);
    return;
}
...
ParallelKDtree<point>::serial_build_recursive(slice In, slice Out, dim_type dim,
    ...
    if (splitIter <= In.begin() + n / 2) {
        ...
        if (_split_rule ==
            ...
            points_iter compIter = diffEleIter == In.begin()
                                       ? std::ranges::prev(In.end())
                                       : In.begin();
    ...
    L = serial_build_recursive(In.cut(0, splitIter - In.begin()),
                               Out.cut(0, splitIter - In.begin()), d, DIM,
                               lbox);
    R = serial_build_recursive(In.cut(splitIter - In.begin(), n),
                               Out.cut(splitIter - In.begin(), n), d, DIM,
                               rbox);
        ...
        [&](bucket_type i) {
            ...
            treeNodes[nodesMap[i]] =
                build_recursive(Out.cut(start, start + sums[nodesMap[i]]),
                                In.cut(start, start + sums[nodesMap[i]]), dim,
                                DIM, boxs[nodesMap[i]]);
        },
        1);

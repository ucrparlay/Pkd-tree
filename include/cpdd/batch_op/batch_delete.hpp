...
void ParallelKDtree<point>::batchDelete(slice A, const dim_type DIM, FullCoveredTag) {
    ...
    std::tie(this->root, this->bbox) = batchDelete_recursive(T, A, parlay::make_slice(B), d, DIM, 1, FullCoveredTag());
    return;
}
...
ParallelKDtree<point>::delete_inner_tree(bucket_type idx, const node_tags& tags, parlay::sequence<node_box>& treeNodes,
                                         bucket_type& p, const tag_nodes& rev_tag, dim_type d, const dim_type DIM) {
...
// tree
template<typename point>
typename ParallelKDtree<point>::node_box ParallelKDtree<point>::batchDelete_recursive(node* T, slice In, slice Out,
    ...
    if (In.size() <= SERIAL_BUILD_CUTOFF) {
        ...
        auto [L, Lbox] =
            batchDelete_recursive(TI->left, In.cut(0, _2ndGroup.begin() - In.begin()),
                                  Out.cut(0, _2ndGroup.begin() - In.begin()), nextDim, DIM, hasTomb, FullCoveredTag());
        auto [R, Rbox] =
            batchDelete_recursive(TI->right, In.cut(_2ndGroup.begin() - In.begin(), n),
    ...
    parlay::parallel_for(
        ...
        [&](size_t i) {
            ...
            treeNodes[i] = batchDelete_recursive(IT.tags[IT.rev_tag[i]].first, Out.cut(start, start + IT.sums[i]),
...
typename ParallelKDtree<point>::node_box ParallelKDtree<point>::batchDelete_recursive(node* T, slice In, slice Out,
    ...
    if (In.size() <= SERIAL_BUILD_CUTOFF) {
        ...
        //   return rebuild_single_tree(T, d, DIM, false, false);
        // }

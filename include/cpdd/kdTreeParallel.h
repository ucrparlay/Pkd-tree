...
class ParallelKDtree {
   public:
    ...
    struct logger {
        size_t vis_leaf_node = 0;
        size_t vis_inter_node = 0;
        size_t jump_node = 0;
        size_t flatten_node = 0;
    };
    ...
    template<typename StoreType>
    size_t range_query_serial(const box& queryBox, StoreType Out, logger& log);
    template<typename StoreType>
    size_t
    range_query_parallel(const typename ParallelKDtree<point>::box& queryBox,
                         StoreType Out, double& tim);
    template<typename StoreType>
    static void range_query_recursive_serial(node* T, StoreType Out, size_t& s,
                                             const box& queryBox,
                                             const box& nodeBox, logger& log);
    template<typename StoreType>

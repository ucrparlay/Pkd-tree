...
struct search
{
   ...
   search( node* T, point p, int k )
       : p( p ), k( k ),
         candidates( parlay::sequence<neighbor>( k, neighbor{ -1, inf } ) )
   {
             ...
             ( T->parent != nullptr ) )
      {
         ...
         if( T == pI->right )
            k_nearest_down( pI->left );
         else
            k_nearest_down( pI->right );
         T = parent;
      }
   }
   ...
   within_epsilon_box( node* T, double epsilon )
   {
      ...
      for( i = 0; i < dims; i++ )
      {
         result = ( result && ( T->bounds.first[i] - epsilon < p.pnt[i] ) &&
...
// **************************************************************
void
process_points_recursive( node* T, knn_graph& knn, int k )
{
   if( T->is_leaf )
   {
      for( int i = 0; i < T->size; i++ )
      {
         ...
         knn[p.id] = parlay::map(
             s.candidates, []( auto x ) { return x.first; }, k );
      }
   }
   else
   {
...
auto
build_knn_graph( const parlay::sequence<coords>& P, int k )
{
   node* T = z_tree( P );
   knn_graph r( P.size() );

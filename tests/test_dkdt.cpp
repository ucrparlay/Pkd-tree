...
template<int D>
struct tree_desc{
  using type = pargeo::dynKdTree::rootNode<D, PointType<coord,D>>;
};
...
main( int argc, char* argv[] ) {
  ...
  auto run = [&](auto dim_wrapper){
    ...
    auto pts = parlay::tabulate( N, [&]( size_t i ) -> PointType<coord, D> {
      return PointType<coord, D>( wp[i].coords() );
    } );
    decltype( wp )().swap( wp );
    testParallelKDtree< tree_desc<D> >(Dim, LEAVE_WRAP, pts, N, K, rounds,

...
void
testParallelKDtree( const int& Dim, const int& LEAVE_WRAP, parlay::sequence<point>& wp, const size_t& N, const int& K,
    ...
    if ( queryType & ( 1 << 2 ) ) {           //* range query
        if ( !( queryType & ( 1 << 1 ) ) ) {  //* run range count to obtain max candidate size
            ...
            parlay::parallel_for( 0, queryNum, [&]( size_t i ) {
                box queryBox = pkd.get_box( box( wp[i], wp[i] ), box( wp[( i + wp.size() / 2 ) % wp.size()],
                                                                      wp[( i + wp.size() / 2 ) % wp.size()] ) );
                kdknn[i] = pkd.range_count( queryBox );
            } );
        }
        //* reduce
        auto maxReduceSize = parlay::reduce( parlay::delayed_tabulate( queryNum, [&]( size_t i ) { return kdknn[i]; } ),
...
main( int argc, char* argv[] ) {
    ...
    if ( tag == -1 ) {
        ...
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> PointID<coord, 2> { return PointID<coord, 2>( wp[i].pnt.begin(), i ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<PointID<coord, 2>>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile, tag, queryType );
    } else if ( Dim == 3 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> PointID<coord, 3> { return PointID<coord, 3>( wp[i].pnt.begin(), i ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<PointID<coord, 3>>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile, tag, queryType );
    } else if ( Dim == 5 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> PointID<coord, 5> { return PointID<coord, 5>( wp[i].pnt.begin(), i ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<PointID<coord, 5>>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile, tag, queryType );
    } else if ( Dim == 7 ) {
        auto pts = parlay::tabulate(
            N, [&]( size_t i ) -> PointID<coord, 7> { return PointID<coord, 7>( wp[i].pnt.begin(), i ); } );
        decltype( wp )().swap( wp );
        testParallelKDtree<PointID<coord, 7>>( Dim, LEAVE_WRAP, pts, N, K, rounds, insertFile, tag, queryType );
    }
